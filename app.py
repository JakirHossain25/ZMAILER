...
MAX_WORKERS = int(os.environ.get('MAX_WORKERS', 3))  # Reduced for stability
EMAIL_SEND_TIMEOUT = int(os.environ.get('EMAIL_SEND_TIMEOUT', 60))
...
def send_via_smtp(sender_email, sender_password, smtp_host, smtp_port, to_email, subject, body, html_body, attachments, sender_name):
    try:
        ...
        try:
            ...
            return False, f"Network error: Cannot connect to {smtp_host}:{smtp_port} - {str(e)}"
        ...
        if smtp_port == 465:
            ...
            with smtplib.SMTP_SSL(smtp_host, smtp_port, context=context, timeout=EMAIL_SEND_TIMEOUT) as server:
        ...
        return False, "SMTP Connection failed - Server may be blocking the port"
...
def upload_emails():
    ...
    try:
        ...
        for line in lines:
            found_emails = re.findall(email_pattern, line.strip().lower())
...
def upload_attachment():
    ...
    try:
        random_filename = generate_random_filename(os.path.splitext(file.filename)[1].lower())
